...
class _SessionsScreenState extends State<SessionsScreen> {
  ...
  Future<void> _loadSessions() async {
    ...
    if (prefs.containsKey(_storageKey)) {
      setState(() {
        sessions = List<Map<String, dynamic>>.from(jsonDecode(prefs.getString(_storageKey)!));
      });
    }
  ...
  void _showSessionDialog({int? index}) {
    ...
    final TextEditingController nameController = TextEditingController(text: isEditing ? session!['title'] : '');
    final TextEditingController playersController = TextEditingController(text: isEditing ? session!['jogadores'].toString() : '5');
    final TextEditingController timeController = TextEditingController(text: isEditing ? session!['duration'].toString() : '8');
                ...
                children: [
                  ...
                  TextField(
                    ...
                    decoration: const InputDecoration(
                      ...
                      enabledBorder: UnderlineInputBorder(borderSide: BorderSide(color: Colors.white24)),
                      focusedBorder: UnderlineInputBorder(borderSide: BorderSide(color: AppColors.accentBlue)),
                      suffixIcon: Icon(Icons.calendar_month, color: AppColors.accentBlue),
                    ),
                    onTap: () async {
                    ...
                    child: ElevatedButton(
                      ...
                      onPressed: () {
                        if (nameController.text.trim().isEmpty) {
                          ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("Preencha o nome!")));
                          return;
                        }

                        setState(() {
                          final newSessionData = {
                            'id': isEditing ? session!['id'] : 'session_${DateTime.now().millisecondsSinceEpoch}',
                            'title': nameController.text.trim(),
                            'date': dateController.text.trim(), // Visually: 12/03/2026
                            'timestamp': selectedDate.toIso8601String(), // --- NEW: Raw data for filtering: 2026-03-12T00:00:00 ---
                            'status': isEditing ? session!['status'] : 'Em Andamento',
  ...
  void _deleteSession(int index) {
    showDialog(
      ...
      builder: (ctx) => AlertDialog(
        backgroundColor: AppColors.headerBlue,
        title: const Text("Excluir Pelada?", style: TextStyle(color: AppColors.textWhite)),
        content: const Text("Tem certeza que deseja excluir esta pelada?", style: TextStyle(color: Colors.white70)),
        actions: [
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: isLoading
          ...
          : sessions.isEmpty
          ? const Center(child: Text("Nenhuma pelada criada neste grupo.", style: TextStyle(color: Colors.white54)))
          : ListView.builder(
                      ...
                      child: Container(
                        ...
                        child: ListTile(
                          ...
                          leading: CircleAvatar(
                            ...
                            child: Icon(Icons.sports_soccer, color: isLive ? Colors.white : Colors.white38),
                          ),
                          title: Text(item['title'], style: const TextStyle(color: AppColors.textWhite, fontWeight: FontWeight.bold, fontSize: 18)),
                          subtitle: Padding(
                          ...
                          trailing: PopupMenuButton<String>(
                            ...
                            onSelected: (String value) {
                              if (value == 'edit') _showSessionDialog(index: index);
                              else if (value == 'delete') _deleteSession(index);
                            },
                            itemBuilder: (BuildContext context) => <PopupMenuEntry<String>>[
                              const PopupMenuItem<String>(value: 'edit', child: Row(children: [Icon(Icons.edit, color: Colors.white, size: 20), SizedBox(width: 8), Text('Editar', style: TextStyle(color: Colors.white))])),
                              const PopupMenuItem<String>(value: 'delete', child: Row(children: [Icon(Icons.delete, color: Colors.redAccent, size: 20), SizedBox(width: 8), Text('Excluir', style: TextStyle(color: Colors.redAccent))])),
                            ],
                          ),
                        ),
                      ),
